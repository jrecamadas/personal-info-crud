							...
							<div class="row" :class="{ 'show': navigationCounter == 1 }">
								<div class="col-sm-12">
									<div class="checkbox-wrapper">
										<input type="checkbox" class="checkbox" id="website" name="website" value="website" v-model="questions" @change="checkIfChecked(questions)" />
										<label for="website"><span>Website</span></label>
									</div>
								</div>
								<div class="col-sm-12">
									<div class="checkbox-wrapper">
										<input type="checkbox" class="checkbox" id="mobile" name="mobile" value="mobile" v-model="questions" @change="checkIfChecked(questions)"/>
							...
							<div class="row" :class="{ 'show': navigationCounter == 2 }">
								<div class="col-sm-12">
									<label class="search-wrapper">
										<input type="search" placeholder="Search skills" id="search-field" class="form-control input-sm" v-model="inputSkill"> 
								...
								<div class="col-sm-12">
									<div class="small-container">
										<div class="row">
											<div class="col-sm-6" v-for="(skill, index) in filteredSkills" :key="index">
												<div class="checkbox-wrapper">
													<input type="checkbox" class="checkbox" :id="skill.text" :name="skill.text" :value="skill.text" v-model="skillSelected" @change="checkIfChecked(skillSelected)" />
													<label :for="skill.text" class="small"><span>{{ skill.text }}</span></label>
												</div>
											</div>
										</div>
									</div>
											...
											<div class="slide-wrapper">
												...
												<div class="emp-info">
													...
													<div>
														Position: <br/>
														<div class="three-ellipsis">
															<span class="dev-position" data-trigger="hover" data-toggle="popover" data-placement="auto" :data-popover-content="'#popover-developer-'+index">
													...
													<popover-dialog :popoverId="'popover-developer-'+index">
                                                        ...
                                                        <template slot="body">
                                                            <div class="skill-block">
                                                                <div class="resource-details-content" v-if="employee.skills && employee.skills.data.length">
                                                                    <div v-for="(skill) in employee.skills.data" :key="skill.id" >
                                                                        <div v-if="checkSkill(skill.name)">
	...
	export default {
		...
		data() {
			return {
				...
				isHiddenTwo: true,
				isHiddenThree: true,
				//skills: ['PHP', 'ReactNative','Xamarin','Ruby','.NET','Yii', 'VueJS','Symfony','Python','Zend'],
				questions: [],
				skillSelected: [],
				navigationCounter: 1,
				inputSkill: '',
		...
		computed: {
			...
			filteredSkills: function() {
				return this.allSkills.filter(s => s.text.toUpperCase().includes(this.inputSkill.toUpperCase()));
			},
		},
		async created() {
		...
		methods: {
			...
			async conditionalNavigation() {
				if(this.navigationCounter == 3){
					...
					this.checkIfChecked(this.skillSelected);
					this.next = true;
					this.prev = true;
					this.done = false;
					this.activeHeight = 235;
					this.option.width = '500px';
				} else {
					this.checkIfChecked(this.questions);
					this.next = true;
					this.prev = false;
					this.done = false;
			...
			async getData(){
				await this.getEmployees({
					...
					{ clientTeam: this.$route.params.id },
					{ developerSkills: this.skillSelected.join() },
					{ take: 999999 })
				});
			},

			checkSkill(skill){
				if (this.skillSelected.filter(function(e) { return e.toLowerCase() === skill.toLowerCase() }).length > 0) {
					return true;
				}
				return false;
			},
			...
			async saveEmp(employee, index){
				...
				const data = {
                    ...
                    job_position_id: employee.positions.data[0].position_id
				}
