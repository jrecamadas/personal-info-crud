<?php

namespace App\Policies;

use App\Models\User;
use App\Models\Address;
use App\Policies\BasePolicy;

class AddressPolicy extends BasePolicy
{
    /**
     * Determine whether the user can view the address.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Address  $address
     * @return mixed
     */
    public function view(User $user, Address $address)
    {
        return $this->isAllowed($user, 'address', 'can_view');
    }

    /**
     * Determine whether the user can create addresses.
     *
     * @param  \App\Models\User  $user
     * @return mixed
     */
    public function create(User $user)
    {
        return $this->isAllowed($user, 'address', 'can_add');
    }

    /**
     * Determine whether the user can update the address.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Address  $address
     * @return mixed
     */
    public function update(User $user, Address $address)
    {
        return $this->isAllowed($user, 'address', 'can_edit');
    }

    /**
     * Determine whether the user can delete the address.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Address  $address
     * @return mixed
     */
    public function delete(User $user, Address $address)
    {
        return $this->isAllowed($user, 'address', 'can_delete');
    }

    /**
     * Determine whether the user can restore the address.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Address  $address
     * @return mixed
     */
    public function restore(User $user, Address $address)
    {
        return $this->isAllowed($user, 'address', 'can_edit');
    }

    /**
     * Determine whether the user can permanently delete the address.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Address  $address
     * @return mixed
     */
    public function forceDelete(User $user, Address $address)
    {
        return $this->isAllowed($user, 'address', 'can_delete');
    }
}
